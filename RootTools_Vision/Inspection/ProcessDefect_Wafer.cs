...
namespace RootTools_Vision
{
    public class ProcessDefect_Wafer : WorkBase
    {
        ...
        private void DeleteOutsideDefect(List<Defect> DefectList, int waferCenterX, int waferCenterY, int Radius, int RadiusOffset)
        {
            ...
            {
                // 좌상단
                float distX = Math.Abs(waferCenterX - (float)DefectList[i].p_DefectBox.Left);
                float distY = Math.Abs(waferCenterY - (float)DefectList[i].p_DefectBox.Top);
                ...
                // 좌하단
                distX = Math.Abs(waferCenterX - (float)DefectList[i].p_DefectBox.Left);
                distY = Math.Abs(waferCenterY - (float)DefectList[i].p_DefectBox.Bottom);
                ...
                // 우상단
                distX = Math.Abs(waferCenterX - (float)DefectList[i].p_DefectBox.Right);
                distY = Math.Abs(waferCenterY - (float)DefectList[i].p_DefectBox.Top);
                dist = Math.Sqrt(Math.Pow(distX, 2) + Math.Pow(distY, 2));
        ...
        private List<Defect> MergeDefect(List<Defect> DefectList, int mergeDist)
        {
            ...
            {
                if (DefectList[i].nDefectCode == -123)
                    continue;

                for (int j = i + 1; j < DefectList.Count; j++)
                {
                    Rect defectRect1 = DefectList[i].p_DefectBox;
                    Rect defectRect2 = DefectList[j].p_DefectBox;
                    defectRect1.Inflate(new Size(mergeDist, mergeDist)); // Rect 가로 1 세로 1 만큼 확장

                    if (defectRect1.IntersectsWith(defectRect2) && (DefectList[i].nDefectCode == DefectList[j].nDefectCode))
                    {
                        ...
                        // Merge Defect Info
                        int nDefectCode = DefectList[j].nDefectCode;
                        float fDefectSz = DefectList[i].fSize + DefectList[j].fSize;
                        int fDefectGV = (int)(DefectList[i].fGV * (DefectList[i].fSize / fDefectSz) + DefectList[j].fGV * (DefectList[j].fSize / fDefectSz));
                        ...
                        DefectList[i].SetDefectInfo(sInspectionID, nDefectCode, fDefectSz, fDefectGV, fDefectRight - fDefectLeft, fDefectBottom - fDefectTop
                            , fDefectRelX, fDefectRelY, fDefectLeft, fDefectTop, DefectList[j].nChipIndexX, DefectList[j].nCHipIndexY);

                        DefectList[j].nDefectCode = -123; // Merge된 Defect이 중복 저장되지 않도록...
                    }
                }
