	...
	public class Run_GrabEBR : ModuleRunBase
	{
		...
		public override string Run()
		{
			...
			try
			{
				...
				int trigger = 1;
				int scanSpeed = 10000;// Convert.ToInt32((double)gmEBR.m_nMaxFrame* camHeight * trigger * (double)gmEBR.m_nScanRate/ 100); //5000;
                ...
                double moveEnd = triggerStart - axisR.GetSpeedValue(Axis.eSpeed.Move).m_acc * scanSpeed * 8;   //y 축 이동 시작 지점 
                double moveStart = triggerDest + axisR.GetSpeedValue(Axis.eSpeed.Move).m_acc * scanSpeed * 8;  // Y 축 이동 끝 지점
