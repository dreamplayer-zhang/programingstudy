...
{
	public class SideVision : ModuleBase
	{
		#region ToolBox
		Axis m_axisRotate;
		...
		public class Run_GrabEdge : ModuleRunBase
		{
			...
			public Run_GrabEdge(SideVision module)
			{
				m_module = module;
			...
			{
				//if (m_gmTop == null) return "Grab Mode == null";
			...
			{
				Inspection(m_gmTop.m_memoryData);
				return "OK";
			...
			public void Inspection(MemoryData md)
			{
				SideVision_Inspection inspection = new SideVision_Inspection();
				IntPtr ptrSrc;

				//System.Threading.Tasks.Parallel.For(0, (md.p_sz.Y / m_nInspHeight), i =>
				//{
				//	ptrSrc = (IntPtr)((long)md.GetPtr() + ((long)(m_nInspHeight * md.W * i)));
				//	inspection.Inspection(ptrSrc, Convert.ToInt32(md.W), m_nInspHeight, i, m_nDefectSize);
				//});
				...
				for (int i = 0; i < (md.p_sz.Y / m_nInspHeight); i++)
				{
					ptrSrc = (IntPtr)((long)md.GetPtr() + ((long)(m_nInspHeight * md.W * i)));
					inspection.Inspection(ptrSrc, Convert.ToInt32(md.W), m_nInspHeight, i, m_nDefectSize);
				}
				List <Defect> defects = inspection.GetDefectList();
				ProcessDefect_Wafer process = new ProcessDefect_Wafer();
				List<Defect> MergeDefectList = process.MergeDefect(defects, m_nMergeDist);  // private로 원복

				string sDefectimagePath = @"D:\DefectImage";
				string sInspectionID = DatabaseManager.Instance.GetInspectionID();
				SaveDefectImageTest(Path.Combine(sDefectimagePath, sInspectionID), MergeDefectList, 1 , md);
			...
			public void SaveDefectImageTest(String Path, List<Defect> DefectList, int nByteCnt, MemoryData md)
			{
				Path += "\\";
				...
				unsafe
				{
					...
					for (int i = 0; i < DefectList.Count; i++)
					{
						...
						rect.x = (int)DefectList[i].p_DefectBox.Left;
						rect.y = (int)DefectList[i].p_DefectBox.Top;
						rect.w = (int)DefectList[i].fWidth;
						rect.h = (int)DefectList[i].fHeight;
		...
		public class Run_GrabNotch : ModuleRunBase
		{
			...
			public Run_GrabNotch(SideVision module)
			{
				m_module = module;
				InitModuleRun(module);
			}
		...
		public class Run_GrabAreaLine : ModuleRunBase
		{
			...
			public Run_GrabAreaLine(SideVision module)
			{
				m_module = module;
				InitModuleRun(module);
			}
		...
		{
			SideVision m_module;
			public Run_Rotate(SideVision module)
			{
				m_module = module;
				InitModuleRun(module);
			}
