...
{
    public class WTR_RND : ModuleBase
    {
        #region ToolBox
        RS232 m_rs232;
        ...
        public class Arm : NotifyProperty
        {
            ...
            public Arm(string id, eArm arm, WTR_RND module)
            {
                ...
                m_waferSize = new WaferSize(m_id, true, false);
            }
        ...
        void RunTreeSetup(Tree tree)
        {
            ...
            foreach (IWTRChild child in m_aChild) child.RunTeachTree(tree.GetTree("Teach", false));
        ...
        public class Run_Motion : ModuleRunBase
        {
            ...
            public override void RunTree(Tree tree, bool bVisible, bool bRecipe = false)
            {
                ...
                p_sChildID = tree.Set(p_sChildID, p_sChildID, p_asChildID, "Child ID", "WTR Child ID", bVisible && (p_asChildID.Count > 1));
            }
            ...
            public string p_sChildID
            {
                get
                {
                    if (m_nChildID >= p_asChildID.Count) m_nChildID = p_asChildID.Count - 1; 
                    if (m_nChildID < 0) m_nChildID = 0;
                    return p_asChildID[m_nChildID]; 
                }
                set
                {
                    for (int n = 0; n < p_asChildID.Count; n++)
                    {
                        if (value == p_asChildID[n]) m_nChildID = n; 
                    }
                }
            }

            List<string> _asChildID = new List<string>(); 
            List<string> p_asChildID
            {
                get 
                {
                    IWTRChild child = m_module.GetChild(m_sChild);
                    if (child == null) return _asChildID; 
                    if (child.p_asChildID == null) return _asChildID;
                    return child.p_asChildID; 
                }
            }
                ...
                switch (m_eMotion)
                {
                    ...
                    case eMotion.GetReady:
                        ...
                        if (m_module.m_dicArm[m_eArm].IsWaferExist() == false)
                        {
                            ...
                            if (child.IsWaferExist()) return "WTR Get Error : Wafer Check Sensor Detected at Child = " + child.p_id;
                    ...
                    case eMotion.PutReady:
                        if (m_module.Run(child.IsPutOK(m_nChildID, m_module.m_dicArm[m_eArm].p_infoWafer))) return p_sInfo; 
                        if (m_module.Run(child.BeforePut(m_nChildID))) return p_sInfo; 
                        child.p_bLock = true;
                        if (m_module.Run(m_module.WriteCmd(cmd, posWTR, m_nChildID + 1, (int)m_eArm + 1))) return p_sInfo;
                        if (m_module.Run(m_module.WaitReply(m_module.m_secMotion))) return p_sInfo; 
                        child.p_bLock = false;
                        if (child.IsWaferExist())
                        {
