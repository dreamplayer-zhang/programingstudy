...
namespace Root_Wind.Module
{
    public class Aligner_ATI_AOI
    {
        ...
        int m_gvEdge = 50;
        CPoint m_szMargin = new CPoint(100, 20);
        CPoint m_szSmooth = new CPoint(0, 0);
        CPoint m_szNotch = new CPoint(35, 35);
        void SetParam(Param param)
        {
            m_gvEdge = param.m_gvEdge;
            m_szMargin = param.m_szMargin;
            m_szSmooth = param.m_szSmooth;
            m_szNotch = param.m_szNotch;
        }
        #endregion
        ...
        public class Notch
        {
            ...
            public void MakePoints(double a, double c, int h, int yOffset, int[] aCalc)
            {
                double fSum = 0;
                double dSum = 0;
                m_aPoints.Clear();
        ...
        public class AOI
        {
            ...
            public bool[] m_abEdge = new bool[1];
            public int[] m_aEdge = new int[1];
            public int[] m_aCalc = new int[1];
            public int[] m_aCircle = new int[1];
            public void InitBuffer()
            {
                int lX = m_memory.p_sz.X;
                if (m_abEdge.Length != lX) m_abEdge = new bool[lX];
                if (m_aEdge.Length != lX) m_aEdge = new int[lX];
                if (m_aCalc.Length != lX) m_aCalc = new int[lX];
                if (m_aCircle.Length != lX) m_aCircle = new int[lX];
            }
        ...
        public void StartInspect(AOI aoi, Param param)
        {
            aoi.m_param = param;
            aoi.m_maxNotch = null; 
            m_qAOI.Enqueue(aoi);
        ...
        {
            SetParam(aoi.m_param);
            m_szMem = aoi.m_memory.p_sz;
            m_xROI = new int[2] { m_szMargin.X, m_szMem.X - m_szMargin.X };
        ...
        void Smooth()
        {
            if (m_szSmooth.X > 0) Parallel.For(0, c_lSmoothThread, y => SmoothX(y));
            if (m_szSmooth.Y > 0) Parallel.For(0, c_lSmoothThread, x => SmoothY(x));
        }
        ...
        {
            int nSum = 0;
            int yMargin = m_szMargin.Y;
            int ySmooth = m_szSmooth.Y;
        ...
        void FindEdgeSub(int x, int xn)
        {
            ...
            while (y <= m_aEdge[xn])
            {
                if ((aBuf[p] >= m_gvEdge) && (aBuf[pn] == 255))
                {
                    m_bFindEdge = true;
                    while (aBuf[p] >= m_gvEdge)
                    {
                        m_aEdge[x] = y;
                        aBuf[p] = 255;
                        y++;
        ...
        void FindCalc()
        {
            ...
            m_xCalc = new int[2] { m_xROI[0] + m_szNotch.X, m_xROI[1] - m_szNotch.X };
            for (int x = m_xCalc[0]; x < m_xCalc[1]; x++)
            {
                m_aCalc[x] = 2 * m_aEdge[x] - (m_aEdge[x - m_szNotch.X] + m_aEdge[x + m_szNotch.X]);
            }
        }
        #endregion
        ...
        void FindNotch()
        {
            ...
            m_aNotch.Clear();
            if (m_abEdge.Length != m_szMem.X) m_abEdge = new bool[m_szMem.X];
        ...
        int FindNotchX(int xMax, int nTry)
        {
            ...
            double sumY = 0;
            double sumXY = 0;
            for (int x = xMax - m_szNotch.X; x <= xMax + m_szNotch.X; x++)
            {
        ...
        void CalcNotch()
        {
            foreach (Notch notch in m_aNotch)
            {
                ...
                double dy = Math.Abs(y0 - m_szNotch.Y);
                double h = 0.15 * m_szNotch.Y;
                if (dy > h)
                {
                    dy = h * Math.Sqrt(dy / h);
                    y0 = (y0 < m_szNotch.Y) ? y0 - dy : y0 + dy;
                }
                double a = -1.0 * y0 / m_szNotch.Y / m_szNotch.Y;
                notch.MakePoints(a, y0, m_szNotch.Y, c_hCalc, m_aCalc);
            }
